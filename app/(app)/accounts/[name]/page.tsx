  ...
  if (user.email === DEMO_EMAIL) {
    ...
    return (
      <AccountPage
        ...
        demo={{
          transcripts: Object.fromEntries(Object.entries(DEMO_TRANSCRIPTS).filter(([k]) => k.startsWith(`${accountName}::`)).map(([k, v]) => [k.split('::')[1], v])),
          threads: Object.fromEntries(Object.entries(DEMO_THREADS).filter(([k]) => k.startsWith(`${accountName}::`)).map(([k, v]) => [k.split('::')[1], v])),
          people: DEMO_PEOPLE[accountName], contracts: DEMO_CONTRACTS[accountName], extra: DEMO_EXTRA[accountName], comms: DEMO_COMMS[accountName], timeline: DEMO_TIMELINE[accountName], riskLines: DEMO_RISK_LINES[accountName] }}
      />
    )
  }
