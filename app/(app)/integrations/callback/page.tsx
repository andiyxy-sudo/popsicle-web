...
const LABEL: Record<string, string> = { gmail: 'Gmail', gcal: 'Google Calendar & Meet', slack: 'Slack', zoom: 'Zoom', hubspot: 'HubSpot', fireflies: 'Fireflies' }
...
function CallbackInner() {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'var(--d-paper, #F8F5F0)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Outfit',sans-serif" }}>
      <div style={{ width: 380, background: 'var(--d-raised, #fff)', border: '1px solid #F0EDE8', borderRadius: 18, padding: 36, textAlign: 'center', boxShadow: '0 16px 48px rgba(15,12,9,.12)' }}>
        ...
        <div style={{ fontSize: 18, fontWeight: 800, color: 'var(--d-ink, #4A3C32)', marginBottom: 6 }}>
          {ok ? `${name} connected` : `${name} connection failed`}
        </div>
        <div style={{ fontSize: 13, color: 'var(--d-muted, #8A7C70)', lineHeight: 1.5 }}>
          {ok
...
export default function IntegrationCallbackPage() {
  return (
    <Suspense fallback={<div style={{ position: 'fixed', inset: 0, background: 'var(--d-paper, #F8F5F0)' }} />}>
      <CallbackInner />
    </Suspense>
  )
}
